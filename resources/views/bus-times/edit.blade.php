                    ...
                    <form action="{{ route('bus-times.update', $busTime) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="frequency" id="frequency" 
                                    ...
                                    <option value="daily" {{ old('frequency', $busTime->frequency) == 'daily' ? 'selected' : '' }}>يومي</option>
                                    <option value="weekdays" {{ old('frequency', $busTime->frequency) == 'weekdays' ? 'selected' : '' }}>أيام الأسبوع فقط</option>
                                    <option value="weekends" {{ old('frequency', $busTime->frequency) == 'weekends' ? 'selected' : '' }}>عطلات نهاية الأسبوع فقط</option>
                                </select>
                                @error('frequency')
                                    <p class="mt-1 text-sm text-red-600 flex items-center">
                                        <i class="fas fa-exclamation-circle mr-1"></i>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border">{{ old('notes', $busTime->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i>
                                    {{ $message }}
